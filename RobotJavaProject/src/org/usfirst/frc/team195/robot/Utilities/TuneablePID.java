...
public class TuneablePID {
	...
	private double pidSendThreadControlStart;
	private double pidSendThreadControlEnd;
	private int pidSendThreadControlElapsedTimeMS;

	private double pidReceiveThreadControlStart;
	private double pidReceiveThreadControlEnd;
	private int pidReceiveThreadControlElapsedTimeMS;
		...
		public void run() {
			while (runThread) {
				...
				do {
					...
					if (pidSendThreadControlElapsedTimeMS < MIN_PID_SEND_RATE_MS)
						try {Thread.sleep(MIN_PID_SEND_RATE_MS - pidSendThreadControlElapsedTimeMS);} catch (Exception ex) {};
				} while (pidSendThreadControlElapsedTimeMS < MIN_PID_SEND_RATE_MS);
			}
		}
		...
		public void run() {
			while (runThread) {
				...
				do {
					...
					if (pidReceiveThreadControlElapsedTimeMS < MIN_PID_RECEIVE_RATE_MS)
						try {Thread.sleep(MIN_PID_RECEIVE_RATE_MS - pidReceiveThreadControlElapsedTimeMS);} catch (Exception ex) {};
				} while (pidReceiveThreadControlElapsedTimeMS < MIN_PID_RECEIVE_RATE_MS);
			}
		}
